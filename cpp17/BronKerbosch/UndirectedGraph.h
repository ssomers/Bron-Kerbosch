...
public:
    ...
    unsigned order() const { return unsigned(itsAdjacencies.size()); }
    unsigned size() const {
        size_t total = 0;
    ...
    unsigned degree(Vertex v) const { return unsigned(itsAdjacencies[v].size()); }
    VertexSet const& neighbours(Vertex v) const { return itsAdjacencies[v]; }
