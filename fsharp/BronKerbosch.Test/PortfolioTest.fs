...
let bk (graph: UndirectedGraph, cliques: int array list) : Unit =
    let expected_cliques = cliques |> List.map (fun c -> c |> Array.map Verticise.it)

    for algo in Portfolio.all_algos do
