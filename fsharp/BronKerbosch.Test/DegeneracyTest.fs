...
let order4_size3_bus () =
    ...
    Assert.That(Degeneracy.iter g |> Seq.head, Is.AnyOf [| { index = 0 }; { index = 3 } |])

[<Test>]
let order4_size3_star () =
    let g = graph_order4_size3_star
    Assert.That(Degeneracy.iter g |> Seq.length, Is.EqualTo 3)
    Assert.That(Degeneracy.iter g |> Seq.head, Is.Not.EqualTo { index = 0 })

[<Test>]
let order4_size4_p () =
    let g = graph_order4_size4_p
    Assert.That(Degeneracy.iter g |> Seq.length, Is.EqualTo 3)
    Assert.That(Degeneracy.iter g |> Seq.head, Is.EqualTo { index = 0 })
...
let order4_size5 () =
    ...
    Assert.That(Degeneracy.iter g |> Seq.length, Is.EqualTo 3)
    Assert.That(Degeneracy.iter g |> Seq.head, Is.AnyOf [| { index = 1 }; { index = 3 } |])
...
let sample () =
    ...
    Assert.That(Degeneracy.iter g |> Seq.head, Is.AnyOf [| { index = 6 }; { index = 7 } |])
...
let bigger () =
    ...
    Assert.That(Degeneracy.iter g |> Seq.head, Is.AnyOf [| { index = 5 }; { index = 8 } |])
