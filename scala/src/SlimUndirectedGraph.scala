...
class SlimUndirectedGraph(neighboursByNode: Adjacencies) extends UndirectedGraph {
    ...
    override def connected_nodes(): mutable.Set[Vertex] = {
        val set = neighboursByNode.zipWithIndex.filter { case (n, v) => !n.isEmpty; }.map { case (n, v) => v; }.toSet
