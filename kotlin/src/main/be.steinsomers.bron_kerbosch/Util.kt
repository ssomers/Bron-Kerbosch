...
object Util {
    ...
    fun append(head: IntArray, tail: Vertex): IntArray =
        head.copyOf(newSize = head.size + 1) { tail.index }
    ...
    fun <T> intersect(set1: Set<T>, set2: Set<T>): MutableSet<T> =
        (if (set1.size <= set2.size) set1 intersect set2 else set2 intersect set1).toMutableSet()

    fun <T> overlap(set1: Set<T>, set2: Set<T>): Int =
        if (set1.size <= set2.size) set1.count(set2::contains) else set2.count(set1::contains)

    fun <T> areDisjoint(set1: Set<T>, set2: Set<T>): Boolean =
        if (set1.size <= set2.size) set1.none(set2::contains) else set2.none(set1::contains)
