...
internal class DegeneracyOrderingTest {
    private fun sortedDegeneracyOrdering(g: UndirectedGraph, drop: Int): SortedSet<Int> {
        ...
        DegeneracyOrdering(g, drop).forEachRemaining(IntConsumer { v: Int ->
    ...
    fun empty() {
        ...
        Assertions.assertTrue(sortedDegeneracyOrdering(g, 0).isEmpty())
        Assertions.assertTrue(sortedDegeneracyOrdering(g, -1).isEmpty())
    }

    @Test
    fun pair() {
        val g = UndirectedGraph(listOf(setOf(1), setOf(0)))
        Assertions.assertEquals(setOf(0, 1), sortedDegeneracyOrdering(g, 0))
        Assertions.assertEquals(1, sortedDegeneracyOrdering(g, -1).size)
        Assertions.assertEquals(0, sortedDegeneracyOrdering(g, -2).size)
    }

    @Test
    fun split() {
        val g = UndirectedGraph(listOf(setOf(1), setOf(0, 2), setOf(1)))
        Assertions.assertNotEquals(1, DegeneracyOrdering(g, 0).next())
        Assertions.assertEquals(setOf(0, 1, 2), sortedDegeneracyOrdering(g, 0))
    }
    ...
    fun degeneracyOrderingCoversConnectedVertices(
        ...
        val connectedVertices: SortedSet<Int> =
            g.connectedVertices().collect(Collectors.toCollection(Supplier { TreeSet() }))
        return sortedDegeneracyOrdering(g, 0) == connectedVertices
    }
    ...
    fun degeneracyOrderingDrops1(
        ...
        val g = UndirectedGraph(adjacencies)
        val ordering = DegeneracyOrdering(g, 0).stream().toArray()
        val ordering1 = DegeneracyOrdering(g, -1).stream().toArray()
    ...
    fun degeneracyOrderingStartsWithLowestDegree(
        ...
        val adjacencies = makeSymmetricAdjacencies(adjacencyLikes)
        val g = UndirectedGraph(adjacencies)
        val ordering = DegeneracyOrdering(g, 0)
        val first: Int
        try {
